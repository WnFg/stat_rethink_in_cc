...
1. **撰写 body 片段** `progress/reviews/chNN_card_body.html`，用 `tools/render_card.py` 文档化的 CSS 类（card/pill/flow/vocab/formula/tag/table/wrong/right…）。必含 4 段：
   - **① 知识总结**：主轴流程图（`.flow`）+ 核心词汇（`.vocab`）+ 关键公式（`.formula`）+ 引擎/对比表 + 本章难点。
   - **② 内嵌可视化**：`<figure><img src="materials/figures/xx.png">`（渲染器会自动 base64 内嵌）。
   - **③ ⭐学习画像段**（取自 `learner-profile.md` + 本章表现）：`.pill.idea` 高光与兴趣、`.pill.warn` 易错点（`.wrong`→`.right` 红绿对照，对应 error-log active）、待强化、`.pill.good` 已攻克。
   - **④ 速查卡**：可直接抄的公式 + 代码（`pre`）。
2. **渲染并打开**：
   ```
