...
| 2026-06-21 | 三区间之和≈1 是采样误差相互抵消 | 集合覆盖：[0,0.5)+[0.5,0.75)+[0.75,1.01)无重叠无遗漏，每个样本只落入一个，和精确=1，与采样无关 | misconception | 1 | active |
| 2026-06-22 | 随机游走步长设为离散 ±1（硬币） | 题目规范为连续 Uniform(-1,1)；两者都收敛正态，但方差公式不同（离散 ±1 时 Var=1，连续时 Var=1/3） | code-bug | 1 | active |
